...
* refactor: Modify "BCG_3metmemb_xtab_METRICSORT.csv" for alternate rules, Issue #18
...
* refactor: Add "BCG_3metmemb_xtab_METRICSORT.csv" to results, Issue #18
...
* fix: Update FIBI low end scoring to use ExclSchool for ni_total and nt_total
...
* refactor: Update "delt" to "delt_ExclSchool" to avoid confusion
    + Calculations are "delt_ExclSchool" but this wasn't in the column names
...
* fix: Update FIBI transformation code
    + Update classes 8 and 9 to 10 and 11
        - Fixed the overall index earlier but not the individual metric transformations
...
* fix: Update index scoring number of metrics, Fish IBI, classes 10 and 11
...
* refactor: New version of BioMonTools
    + Edits to MN_IBI_Bugs and MN_IBI_Fish index numeric and narrative thresholds
...
* fix: Use same transformation for all regions for pi_TrichNoHydro metric
...
* refactor: Added to BCG calc in Shiny QC check for non-consecutive Levels
...
* refactor: Update 2 tables in FB_TaxaTrans_1About from knitr to flextable
...
* refactor: Updated language in multiple UI elements per stylistic edits.

# MNcalc 0.2.0.9032 (2024-07-25)

* refactor: IBI slim down columns to only those for IBI for _RESULTS file
...
* refactor: Modify Shiny file builder, taxa translate to combine duplicate taxa
