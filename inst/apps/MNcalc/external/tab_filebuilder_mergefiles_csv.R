...
function() {
  sidebarLayout(
    sidebarPanel(
      ...
      , shinyjs::disabled(shinyBS::bsButton("b_calc_mergefiles_csv"
                                            , label = "Run Operation"))

      , h4("D. Download Output")
      , p("All input and output files will be available in a single zip file.")
      , shinyjs::disabled(downloadButton("b_download_mergefiles_csv"
                                         , "Download Results"))
    ...
    , mainPanel(
      includeHTML(file.path("www"
                            , "rmd_html"
                            , "ShinyHTML_FB_MergeFiles_csv_1About.html"))
    )## mainPanel ~ END
  )##sidebarLayout ~ END
}##FUNCTION ~ END
